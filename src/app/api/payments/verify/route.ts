...
export async function GET(req: Request) {
  ...
  if (!transactionId || status !== 'successful') {
    return NextResponse.redirect(`${appUrl}/payment-history?status=failed`, { status: 302 });
  }
  ...
  try {
    ...
    return NextResponse.redirect(`${appUrl}/payment-history?status=success`, { status: 302 });
  } catch (err) {
    return NextResponse.redirect(`${appUrl}/payment-history?status=failed`, { status: 302 });
  }
}
